...
CLOSURE_SAMPLE_PROFILE = str(
    globals().get("CLOSURE_SAMPLE_PROFILE")
    or os.environ.get("CLOSURE_SAMPLE_PROFILE", "full")
).strip().lower()
if CLOSURE_SAMPLE_PROFILE not in ("major", "full"):
    raise RuntimeError("CLOSURE_SAMPLE_PROFILE must be major or full")
if CLOSURE_SAMPLE_PROFILE == "major" and "SAMPLE_FILTER" not in os.environ:
    _sample_to_group = {}
    for _group_name, _group_cfg in _full_config["plot_groups"].items():
        for _output_name in _group_cfg.get("samples", ()):
            _sample_to_group[_output_name] = _group_name
    _major_groups = {"DY", "ZZ", "WZ", "Vg", "VgS", "top", "ttV_tZ"}
    _major_outputs = [
        _output_name
        for _output_name in _resolved_overlap["output_names"]
        if _sample_to_group.get(_output_name) in _major_groups
    ] + ["DATA"]
    os.environ["SAMPLE_FILTER"] = ",".join(_major_outputs)
