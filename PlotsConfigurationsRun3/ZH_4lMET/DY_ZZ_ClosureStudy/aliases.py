...
_L2_GATE_INCLUDE = [
    '#include "%s/PlotsConfigurationsRun3/ZH_4lMET/DY_ZZ_ClosureStudy/macros/four_lepton_helpers.cc"'
    % configurations
]
...
_trigger_adapter_dir = (
    configurations + "PlotsConfigurationsRun3/ZH_4lMET/DY_ZZ_ClosureStudy"
)
...
_SELECTED_TRIGGER_INCLUDE = [
    '#include "%s/PlotsConfigurationsRun3/ZH_4lMET/DY_ZZ_ClosureStudy/macros/selected_trigger_wrappers.cc"'
    % configurations
]
...
_BTAG_INCLUDE = [
    '#include "%s/PlotsConfigurationsRun3/ZH_4lMET/DY_ZZ_ClosureStudy/macros/fixed_wp_btag_sf.cc"'
    % configurations
]
aliases["bVeto"] = {
...
# Closure-only helpers use exactly the vendored tight WPs and selected Z/X
# objects above; no second pairing or object-ID implementation is maintained.
_closure_include = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "macros", "closure_helpers.cc"
)
_closure_tight_ele = f"Lepton_isTightElectron_{PAIR_ELE_WP}"
_closure_tight_mu = f"Lepton_isTightMuon_{PAIR_MU_WP}"
aliases["PassesAnchor2lPt"] = {
    "linesToAdd": [f'#include "{_closure_include}"'],
    "expr": (
        "ClosureBridge::passesAnchor2lPt(Lepton_pt, Lepton_pdgId, "
        f"{_closure_tight_ele}, {_closure_tight_mu}, 25.f, 15.f)"
    ),
}
aliases["nExtraTight10"] = {
    "expr": (
        "ClosureBridge::nExtraTight10(Lepton_pt, Lepton_pdgId, "
        f"{_closure_tight_ele}, {_closure_tight_mu}, Z0_idx)"
    ),
}
aliases["Z0_absRapidity"] = {
    "expr": "ClosureBridge::safeAbsRapidity(Z0_pt, Z0_eta, Z0_phi, Z0_mass)"
}
aliases["phiEtaStar"] = {
    "expr": (
        "ClosureBridge::phiEtaStar("
        "Alt(Lepton_eta, Alt(Z0_idx,0,-1), -999.f), "
        "Alt(Lepton_phi, Alt(Z0_idx,0,-1), -999.f), "
        "Alt(Lepton_eta, Alt(Z0_idx,1,-1), -999.f), "
        "Alt(Lepton_phi, Alt(Z0_idx,1,-1), -999.f))"
    )
}
aliases["Z_lead_pt"] = {
    "expr": "max(Alt(Lepton_pt, Alt(Z0_idx,0,-1), -999.f), Alt(Lepton_pt, Alt(Z0_idx,1,-1), -999.f))"
}
aliases["Z_sublead_pt"] = {
    "expr": "min(Alt(Lepton_pt, Alt(Z0_idx,0,-1), -999.f), Alt(Lepton_pt, Alt(Z0_idx,1,-1), -999.f))"
}
aliases["Z_lead_absEta"] = {
    "expr": "ClosureBridge::selectedAbsEta(Lepton_pt, Lepton_eta, Z0_idx, true)"
}
aliases["Z_sublead_absEta"] = {
    "expr": "ClosureBridge::selectedAbsEta(Lepton_pt, Lepton_eta, Z0_idx, false)"
}
aliases["nJet30"] = {"expr": "Sum(CleanJet_pt > 30.)"}
aliases["nBLoose"] = {
    "expr": (
        "Sum(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && "
        f"Take(Jet_{btag_veto_algo}, CleanJet_jetIdx) > {btag_veto_WP})"
    )
}
