...
CORE_SHAPES = ("Z0_mass", "Z0_pt", "phiEtaStar", "PV_npvsGood", "nJet30", "PuppiMET_pt", "nExtraTight10", "X_mass", "minSelectedPairMass", "nBLoose")
NMINUS_VARIABLE = {
    "N1_NO_XMASS": "X_mass",
    "N1_NO_XFLAVOR": "X_flavor_code",
    "N1_NO_BVETO": "nBLoose",
    "N1_NO_LOWMASS": "minSelectedPairMass",
    "N1_NO_FIFTHVETO": "nExtraTight10",
    "N1_NO_4LPT": "selected4lPt1",
    "N1_NO_ZWINDOW": "Z0_mass",
}
    ...
    try:
        ...
        for era, samples in all_samples.items():
            if "DATA" not in samples:
                raise ValueError(f"{era} input has no nominal DATA histogram in S8_Z_BRIDGE/yield; is this a merged ROOT file?")
            if not any(sample != "DATA" for sample in samples):
                raise ValueError(f"{era} input has no nominal MC histograms in S8_Z_BRIDGE/yield; is this a merged ROOT file?")
        ...
        for label, eras in combinations.items():
            ...
            for category in categories:
                ...
                variable = preferred if preferred in variables else next(
                    (name for name in ("yield", "Z0_mass", "Z0_pt", "X_mass", *variables) if name in variables),
                    None,
                )
                if variable is None:
                    continue
            ...
            for scope, both_category, current_only_category, event_only_category in (
                ...
                for population, category in (
                    ...
                    dpop = reader.aggregate(eras, category, "Z0_mass", ("DATA",))
                    mpop = reader.aggregate(eras, category, "Z0_mass", mc_samples)
                    if dpop and mpop:
        ...
        if not stage_rows:
            raise RuntimeError("No closure stage metrics were produced from the supplied ROOT files")
