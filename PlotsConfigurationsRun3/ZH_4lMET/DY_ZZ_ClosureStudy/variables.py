...
EDGES = {
    ...
    "Z0_mass": (60, 30.0, 150.0),       # 2 GeV: detector-scale DY mass resolution
    "Z0_pt": (70, 0.0, 140.0),         # 2 GeV: high-statistics DY recoil shape
    ...
    "X_pt": (7, 0.0, 140.0),           # matches coarse four-lepton Z0_pt binning
    "m4l": (26, 80.0, 600.0),          # only coarse categories book this observable
    "minSelectedPairMass": (25, 0.0, 100.0), # 4 GeV; 12 GeV is an exact edge
    "nBLoose": (6, -0.5, 5.5),
...
DEFINITIONS = OrderedDict(
    (
        ...
        ("Z0_mass", _axis("Z0_mass", "Z0_mass", "m_{ll} [GeV]", EDGES["Z0_mass"])),
        ("Z0_pt", _axis("Z0_pt", "Z0_pt", "p_{T}(Z) [GeV]", EDGES["Z0_pt"])),
        ...
        ("minSelectedPairMass", _axis("minSelectedPairMass", "minSelectedPairMass", "min m_{ij} [GeV]", EDGES["minSelectedPairMass"])),
        ("nBLoose", _axis("nBLoose", "nBLoose", "N_{b}^{loose}", EDGES["nBLoose"])),
        ("X_flavor_code", _axis("X_flavor_code", "2*X_isSF + X_isDF", "X flavor (0 other, 1 DF, 2 SF)", EDGES["X_flavor_code"], 0)),
        ("selected4lPt1", _axis("selected4lPt1", "Max(Take(Lepton_pt, Concatenate(Z0_idx, X_idx)))", "selected 4l p_{T}^{1} [GeV]", EDGES["selected4lPt"])),
        ("selected4lPt4", _axis("selected4lPt4", "Min(Take(Lepton_pt, Concatenate(Z0_idx, X_idx)))", "selected 4l p_{T}^{4} [GeV]", EDGES["selected4lPt"])),
    )
)
...
_book(_sentinels, ("yield", "Z0_mass", "Z0_pt", "phiEtaStar", "PuppiMET_pt", "nJet30", "nExtraTight10"))
_book(("S8_Z_BRIDGE",), ("PV_npvsGood",))
_book(_other_primary, ("yield", "Z0_pt", "PuppiMET_pt", "nExtraTight10"))
_book(("S0_ZZCR", "S7_FOURL_BRIDGE"), ("X_mass", "X_pt", "m4l", "minSelectedPairMass", "nBLoose", "selected4lPt1"))

_nminus = {
    "N1_NO_XMASS": ("X_mass", "PuppiMET_pt"),
    "N1_NO_XFLAVOR": ("X_flavor_code", "X_mass"),
    "N1_NO_BVETO": ("nBLoose", "nJet30"),
    "N1_NO_LOWMASS": ("minSelectedPairMass", "X_mass"),
    "N1_NO_FIFTHVETO": ("nExtraTight10", "yield"),
    "N1_NO_4LPT": ("selected4lPt1", "selected4lPt4"),
    "N1_NO_ZWINDOW": ("Z0_mass", "Z0_pt"),
}
for _category, _names in _nminus.items():
    _book((_category,), _names)

_extra_categories = ("S8_EXTRA0", "S8_EXTRA1", "S8_EXTRA2P")
_book(_extra_categories, ("yield", "Z0_mass", "Z0_pt", "phiEtaStar", "PuppiMET_pt", "nJet30"))
...
_book(_flavor_categories, ("Z0_mass", "Z0_pt", "phiEtaStar", "Z_lead_pt", "Z_sublead_pt", "Z_lead_absEta", "Z_sublead_absEta", "PV_npvsGood"))
...
_book(_topology_categories, ("yield", "Z0_mass", "X_mass", "Z0_pt", "PuppiMET_pt", "m4l"))

_migration_categories = ("PT_ENRICHED_CURRENT_ONLY", "PT_ENRICHED_EVENTPT_ONLY", "PT_BROAD_CURRENT_ONLY", "PT_BROAD_EVENTPT_ONLY")
_book(_migration_categories, ("Z0_mass", "Z0_pt", "phiEtaStar", "nExtraTight10", "Z_lead_pt", "Z_sublead_pt"))
...
_book(_trigger_categories, ("yield", "Z0_pt", "Z_lead_pt", "Z_sublead_pt"))
_stream_categories = tuple(
    f"{parent}_{suffix}"
    for parent in ("S8_Z_BRIDGE", "D0_DY_ENRICHED_CURRENT")
    for suffix in ("STREAM_MUONEG", "STREAM_MUON", "STREAM_EGAMMA")
)
_book(_stream_categories, ("yield", "Z0_pt"))
...
for _stage in WEIGHT_SENTINELS:
    ...
    _lep = "SelectedLeptonSF_ZX" if _is_four else "SelectedLeptonSF_Z"
...
if os.environ.get("CLOSURE_PROFILE", "default").strip().lower() == "focused_cross":
    _book(("S8_FOCUSED_ZEE_EGAMMA", "S8_FOCUSED_ZMM_MUON"), ("yield", "Z0_mass", "Z0_pt"))
