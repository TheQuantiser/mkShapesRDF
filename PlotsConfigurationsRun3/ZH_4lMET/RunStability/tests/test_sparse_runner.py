...
from mkShapesRDF.shapeAnalysis.runner import (  # noqa: E402
    RunAnalysis as CoreRunAnalysis,
)
...
def test_overlapping_categories_have_independent_event_weights():
    variables = {
        "x": {"name": "x", "range": (1, 0.5, 1.5), "categories": ["DY_ALL", "DY_ZEE"]}
    }
    cuts = {
        "DY_ALL": {"expr": "true", "parent": "DY"},
        "DY_ZEE": {"expr": "true", "parent": "DY"},
    }
    runner = _bare(RunAnalysis, variables, cuts, {"DY_ALL": "3.f", "DY_ZEE": "5.f"})
    runner.create_cuts_vars()
    assert _integral(runner.results["DY_ALL"]["x"]["MC"][0]) == pytest.approx(6.0)
    assert _integral(runner.results["DY_ZEE"]["x"]["MC"][0]) == pytest.approx(10.0)
...
def test_sparse_filtering_and_unrestricted_core_compatibility():
    ...
    sparse_variables = {
        "x": {"name": "x", "range": (1, 0.5, 1.5), "categories": ["DY_ALL", "DY_ZEE"]},
        "y": {"name": "y", "range": (1, 1.5, 2.5), "categories": ["DY_ZEE"]},
    }
    sparse = _bare(
        RunAnalysis, sparse_variables, cuts, {"DY_ALL": "1.f", "DY_ZEE": "1.f"}
    )
    sparse.create_cuts_vars()
    assert "y" not in sparse.results["DY_ALL"]
    assert _integral(sparse.results["DY_ZEE"]["y"]["MC"][0]) == pytest.approx(2.0)

    unrestricted = {
        name: {key: value for key, value in definition.items() if key != "categories"}
        for name, definition in sparse_variables.items()
    }
...
def test_sparse_variations_survive_conversion_and_save(tmp_path):
    ...
    zz_variations = runner.results["DY_ZEE"]["x"]["MC"][0]
