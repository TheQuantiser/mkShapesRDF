from pathlib import Path
import subprocess


CONFIG_DIR = Path(__file__).resolve().parents[1]
WRAPPER = CONFIG_DIR / "fnal_lpc_packaged_env.sh"


def _source_wrapper(prefix=""):
    command = (
        f"{prefix} source {WRAPPER}; "
        "printf '%s|%s|%s|%s|%s' \"$EXECUTION_PROFILE\" "
        '"$INPUT_ACCESS_MODE" "$XRD_READ_ENDPOINT" '
        '"$XRD_WRITE_ENDPOINT" "$CONDOR_RUNTIME_SETUP"'
    )
    return subprocess.run(
        ["bash", "-c", command],
        check=True,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    ).stdout


def test_fnal_wrapper_defaults_to_direct_xrootd():
    assert _source_wrapper() == (
        "packaged_fnal_xrootd_eos_production|xrootd|"
        "root://eoscms.cern.ch|root://cmseos.fnal.gov|"
        "source /cvmfs/sft.cern.ch/lcg/views/LCG_109/"
        "x86_64-el9-gcc13-opt/setup.sh;;"
        "export X509_VOMS_DIR=/cvmfs/grid.cern.ch/etc/grid-security/vomsdir"
    )


def test_fnal_wrapper_replaces_stale_site_values():
    assert _source_wrapper(
        "export EXECUTION_PROFILE=packaged_fnal_stagein_eos_production "
        "INPUT_ACCESS_MODE=stage-in "
        "XRD_READ_ENDPOINT=root://stale-read.example "
        "XRD_WRITE_ENDPOINT=root://stale-write.example "
        "CONDOR_RUNTIME_SETUP='export X509_VOMS_DIR=/tmp/stale';"
    ) == (
        "packaged_fnal_xrootd_eos_production|xrootd|"
        "root://eoscms.cern.ch|root://cmseos.fnal.gov|"
        "source /cvmfs/sft.cern.ch/lcg/views/LCG_109/"
        "x86_64-el9-gcc13-opt/setup.sh;;"
        "export X509_VOMS_DIR=/cvmfs/grid.cern.ch/etc/grid-security/vomsdir"
    )
