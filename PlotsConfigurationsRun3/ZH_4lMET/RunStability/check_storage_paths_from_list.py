#!/usr/bin/env python3
"""Verify materialized RunStability storage paths against an all-parts inventory.
...
def _build_expectations(
...
) -> Tuple[List[ExpectedSample], Dict[Tuple[str, str], List[int]]]:
    ...
    for era, year_cfg_any in years.items():
        ...
        if not isinstance(samples, list) or not all(
            isinstance(item, str) for item in samples
        ):
        ...
        for sample in samples:
            ...
            directory = _normalize_directory(
                posixpath.join(tree_base, production, steps)
            )
            add_check(
                ExpectedSample(
        ...
        if not isinstance(data, Mapping):
            raise VerificationError(
                f"Era {era!r} has no materialized DATA configuration"
            )
        reco = data.get("reco")
            ...
            directories = tuple(
                dict.fromkeys(
                    (
                        ...
                        _normalize_directory(
                            posixpath.join(tree_base, reco, data_steps)
                        ),
                    )
                )
            )
...
def _stream_inventory(
    ...
    try:
        ...
        raise VerificationError(
            f"Cannot open inventory {inventory_path}: {exc}"
        ) from exc
...
def audit_configuration(
    config: Mapping[str, Any], inventory_path: Path
) -> Dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "kind": "run_stability_all_parts_storage_audit",
