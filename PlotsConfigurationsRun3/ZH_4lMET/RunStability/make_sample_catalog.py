...
The default text-list semantics select one representative file per sample and
processing path: ``part0`` when it exists, otherwise ``part1``.  Nominal paths
are written first, one blank line separates them from systematic paths, and
systematic paths are recognized by a processing suffix ending in ``do_suffix``
or ``up_suffix``.
...
DEFAULT_FIND_REGEX_REPRESENTATIVE = r".*__part[01][.]root$"
...
VARIATION_TOKEN_RE = re.compile(r"^(?P<systematic>.*?)(?P<raw_direction>do|up)_suffix$")
...
class EOSCommandError(CatalogError):
    ...
    def __init__(
        ...
        super().__init__(command, returncode, stderr, attempts)
...
class EOSClient:
    ...
    def list_directory(self, path: str) -> tuple[DirectoryEntry, ...]:
        ...
        return tuple(
            sorted(entries, key=lambda item: (not item.is_directory, item.name))
        )
...
class EOSCrawler:
    ...
    def crawl(self) -> CrawlResult:
        ...
        failures: list[tuple[CampaignRef, Exception]] = []
...
class CatalogBuilder:
    ...
    def build(self) -> dict[str, Any]:
        ...
        for record in sorted(self.records, key=lambda item: item.path):
            ...
            by_year[
                str(record.campaign_year) if record.campaign_year else "unknown"
            ] += 1
        ...
        document: dict[str, Any] = {
            ...
            "quality": {
                "parse_failures": [
                    dataclasses.asdict(item) for item in self.parse_failures
                ],
            ...
            if self.include_replica_details:
                group["copies"] = [
                    ...
                    for record in sorted(
                        records, key=lambda item: (item.owner, item.path)
                    )
                ]
            groups.append(group)
        return sorted(
...
def parse_file_record(path: str, *, base: str) -> FileRecord:
    ...
    if not file_match:
        raise ValueError("filename does not match nanoLatino_<sample>__partN.root")
...
def select_representative_paths(paths: Iterable[str]) -> tuple[str, ...]:
    """Prefer part0, falling back to part1, for each exact file identity.

    The identity retains the full directory and sample filename and removes
    only the terminal part number.  Consequently nominal and systematic
    processing paths, owners, campaigns, and samples are never mixed.
    """

    selected: dict[str, tuple[int, str]] = {}
    for path in sorted(set(paths)):
        match = re.search(r"__part(?P<part>[01])[.]root$", path)
        if match is None:
            raise CatalogError(
                "Representative selection received a path outside parts 0/1: " f"{path}"
            )
        part = int(match.group("part"))
        identity = path[: match.start()] + "__partN.root"
        current = selected.get(identity)
        if current is None or part < current[0]:
            selected[identity] = (part, path)
    return tuple(
        path for _part, path in sorted(selected.values(), key=lambda item: item[1])
    )
...
def _validate_find_regex(regex: str) -> None:
    ...
    try:
        ...
        raise ConfigurationError(
            f"Invalid file-name regular expression {regex!r}: {exc}"
        )
    python_only = re.search(r"\\[dDsSwWAZbB]|[(][?]", regex)
    if python_only:
        raise ConfigurationError(
            "File-name expressions must use the portable Python/ERE subset; "
            f"unsupported construct {python_only.group(0)!r} in {regex!r}"
        )
...
def make_parser() -> argparse.ArgumentParser:
    ...
    part_group.add_argument(
        "--part",
        type=int,
        help="select one exact part number instead of the default part0/part1 fallback",
    )
    part_group.add_argument(
        "--all-parts",
        action="store_true",
    ...
    part_group.add_argument(
        "--find-name-regex",
        help=(
            "advanced portable Python/ERE override for the EOS --name expression; "
            "mutually exclusive with --part and --all-parts"
        ),
    )
    crawl.add_argument(
        "--owner-regex",
...
def run_crawl(args: argparse.Namespace) -> int:
    ...
    if args.find_name_regex:
        ...
        find_regex = DEFAULT_FIND_REGEX_REPRESENTATIVE
    ...
    owner_patterns = [_compile_regex(value, "owner") for value in args.owner_regex]
    ...
    selected_paths = (
        select_representative_paths(result.paths)
        if representative_fallback
        else result.paths
    )
    write_traditional_file_list(list_output, selected_paths, base=args.base)

    crawl_metadata = {
        "mode": "eos_crawl",
        "years": sorted(set(args.years)),
        "find_name_regex": find_regex,
        "representative_part_policy": (
            "prefer_part0_else_part1" if representative_fallback else None
        ),
        "discovered_paths_before_part_selection": len(result.paths),
        "workers": args.workers,
    ...
    if result.recovered_events:
        atomic_write_text(
            errors_output,
            "\n".join(
                json.dumps(item, sort_keys=True) for item in result.recovered_events
            )
            + "\n",
        )
    else:
