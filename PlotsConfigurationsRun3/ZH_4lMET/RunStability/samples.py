...
from mkShapesRDF.lib.remote_io import resolve_input_uri  # noqa: E402
from mkShapesRDF.lib.search_files import SearchFiles  # noqa: E402
...
if _remote_discovery_enabled and not redirector:
    raise RuntimeError(
        "RunStability remote discovery requested but xrdDiscoveryEndpoint is missing"
    )
if _remote_discovery_enabled and not readRedirector:
    raise RuntimeError(
        "RunStability remote discovery requested but xrdReadEndpoint is missing"
    )
...
if "analysis_pass" not in globals() or "selected_correction_weight" not in globals():
    from selection_config import analysis_pass, selected_correction_weight
...
SAMPLE_PROFILE = (
    str(
        ...
        or os.environ.get("SAMPLE_PROFILE", "presentation")
    )
    .strip()
    .lower()
)
...
for mc_sample in _resolved_overlap["passthrough_sources"]:
    ...
    samples[mc_sample] = {
        "name": files,
        "weight": mcCommonWeight + "*" + selected_correction_weight(_PASS),
...
for _process_name, _process_cfg in _resolved_overlap["processes"].items():
    ...
    for _component in _process_cfg["components"]:
        ...
        _source_norm = source_normalization(_source_alias, _selected_year, _full_config)
        _components.append(
            (
                _source_alias,
                _files,
...
_data_run_filter = {
    item.strip()
    for item in os.environ.get("DATA_RUN_FILTER", "").split(",")
    if item.strip()
}
try:
    DataRunTags = resolve_data_run_filter(DataRunTags, _data_run_filter)
except ValueError as _data_run_filter_error:
    raise RuntimeError(
        f"Invalid DATA_RUN_FILTER for YEAR={YEAR}: {_data_run_filter_error}"
    ) from _data_run_filter_error
_data_stream_filter = {
    item.strip()
...
if _sample_enabled("DATA"):
    ...
    for data_sample in DataSamples:
        ...
        sample_run_tags = resolve_data_sample_run_tags(DataRunTags, data_sample)
