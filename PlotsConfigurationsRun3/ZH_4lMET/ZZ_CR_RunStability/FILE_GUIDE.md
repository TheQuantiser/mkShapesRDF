# ZZ_CR run-stability file architecture and customization guide

The copied files retain their established responsibilities unless noted here.
`run_stability_config.py` owns compile-time luminosity validation, exact run
ordering, generated C++ lookup, auxiliary-path inventory, and serialized
metadata. `zz_cr_runner.py` still delegates ordinary TH1 behavior to the
unchanged core runner and additionally owns DATA-only TH2 booking, y-only
folding, run-flow rejection, and merge-safe metadata output. No implementation
for this leaf belongs in mkShapesRDF core or the original `ZZ_CR`.

This document explains the files that participate in the
`PlotsConfigurationsRun3/ZH_4lMET/ZZ_CR_RunStability` workflow, how mkShapesRDF consumes
...
- [`USAGE.MD`](USAGE.MD), which gives the bounded local compile, pilot, and
  inspection commands supported by this leaf;
...
```bash
pytest -q PlotsConfigurationsRun3/ZH_4lMET/ZZ_CR_RunStability/tests
```
...
### Submission sites and output destinations

The inherited wrappers remain available as source context, but this
run-stability implementation has not validated or authorized batch
submission, remote stage-out, or a production destination. `USAGE.MD`
deliberately documents only bounded local-XRootD execution.
...
```bash
cd PlotsConfigurationsRun3/ZH_4lMET/ZZ_CR_RunStability
...
Use the exact bounded compile, pilot, and inspection commands in `USAGE.MD`.
Production submission, stage-out, run projections, luminosity scaling, and
Data/MC plotting remain outside this leaf's validated workflow.
