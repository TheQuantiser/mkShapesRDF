# DY run-stability configuration contract

This leaf inherits the established four-lepton selections, samples, weights,
and ordinary TH1 contract described below. `ANALYSIS_PASS=RUN_STABILITY` is
DY-only. It can resolve either all 25 supported DY observables in all 24
standard DY categories or an exact named subset. The analysis-local
`run_stability_config.py` and `zz_cr_runner.py` add the corresponding DATA-only
TH2 matrix plus run-aligned nominal/`Trigger_Any` luminosity metadata. MC
remains era-inclusive and one-dimensional. See `README.md` for selectors,
schema validation, luminosity scaling, statistical treatment, and on-demand
plot commands. Historical production discussion below is inherited context,
not authorization to submit a new run-stability campaign.
...
after sourcing the wrapper. The packaged FNAL wrapper keeps the established
`ZZ_CR` CERN-XRootD-to-FNAL-EOS transport unchanged and adds a worker runtime
export of `X509_VOMS_DIR=/cvmfs/grid.cern.ch/etc/grid-security/vomsdir` before
the staged proxy is validated. This avoids dependence on worker-local VOMS
issuer files without changing the operator-created proxy or packaging it in
the runtime archive.
