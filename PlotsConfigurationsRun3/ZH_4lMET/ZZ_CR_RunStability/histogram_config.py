...
def build_registry(raw_variables, binning_contract):
    ...
    for name, raw in raw_variables.items():
        ...
        definition["physics_role"] = (
            "default analysis observable"
            if "analysis" in definition["tags"]
...
def materialize_histograms(
    ...
    if profile not in PROFILE_NAMES:
        raise ValueError(
            f"Unknown HISTOGRAM_PROFILE={profile!r}; available={PROFILE_NAMES}"
        )

    required_category_variables = tuple(
        dict.fromkeys(str(name) for name in required_category_variables)
    )
    missing_required = sorted(set(required_category_variables) - set(registry))
    if missing_required:
        raise ValueError(
            "Required category variables are absent from the registry: "
            f"{missing_required}"
        )
    ...
    if exact_required_category_variables and (include or exclude):
        raise ValueError(
            "VARIABLE_INCLUDE/VARIABLE_EXCLUDE cannot be combined with the exact "
            "RUN_STABILITY observable selector"
        )

    if exact_required_category_variables:
        selected = list(required_category_variables)
    elif include:
    ...
    selected.extend(
        name for name in required_category_variables if name not in selected
    )

    policy_names = {
        category_id: set(
            VIEW_VARIABLE_POLICIES[(category["physics_region"], category["view_type"])]
        )
    ...
    for name in selected:
        ...
        allowed = [
            ...
            and (
                bool(include)
                or profile == "all"
                or name in required_category_variables
