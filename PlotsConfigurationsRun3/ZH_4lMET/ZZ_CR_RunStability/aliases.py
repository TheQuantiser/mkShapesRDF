...
if globals().get("RUN_STABILITY_CONTRACT", {}).get("enabled"):
    if DATA_SAMPLES not in ([], ["DATA"]):
        raise RuntimeError(
            "RUN_STABILITY permits either an MC-only selection or exactly one "
            f"logical DATA sample named DATA; received {DATA_SAMPLES}"
        )
    if DATA_SAMPLES:
    ...
    # This marker is removed by the analysis-local runner before the aliases
    # reach mkShapesRDF core.  It carries the complete compiled run/luminosity
    # contract in both local and packaged worker execution.
...
_L2_GATE_INCLUDE = [
    '#include "%s/PlotsConfigurationsRun3/ZH_4lMET/ZZ_CR_RunStability/macros/four_lepton_helpers.cc"'
    % configurations
]
...
_trigger_adapter_dir = (
    configurations + "PlotsConfigurationsRun3/ZH_4lMET/ZZ_CR_RunStability"
)
...
_SELECTED_TRIGGER_INCLUDE = [
    '#include "%s/PlotsConfigurationsRun3/ZH_4lMET/ZZ_CR_RunStability/macros/selected_trigger_wrappers.cc"'
    % configurations
]
...
_BTAG_INCLUDE = [
    '#include "%s/PlotsConfigurationsRun3/ZH_4lMET/ZZ_CR_RunStability/macros/fixed_wp_btag_sf.cc"'
    % configurations
]
aliases["bVeto"] = {
