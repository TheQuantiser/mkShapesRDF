...
ANALYSIS_PASS = str(os.environ.get("ANALYSIS_PASS", "RUN_STABILITY")).strip().upper()
if ANALYSIS_PASS not in ("ALL", "ZPARENT", "FOURL_BASE", "CONTROL", "RUN_STABILITY"):
    raise ValueError(
        "ANALYSIS_PASS must be ALL, ZPARENT, FOURL_BASE, CONTROL, or "
        "RUN_STABILITY; "
        f"received {ANALYSIS_PASS!r}"
    )
os.environ["ANALYSIS_PASS"] = ANALYSIS_PASS

RUN_STABILITY_REGION = str(os.environ.get("RUN_STABILITY_REGION", "DY")).strip().upper()
...
SAMPLE_PROFILE = (
    os.environ.get(
        "SAMPLE_PROFILE",
        "presentation" if ANALYSIS_PASS == "RUN_STABILITY" else "commissioning",
    )
    .strip()
    .lower()
)
...
if ANALYSIS_PASS == "RUN_STABILITY":
    required = {
        "CATEGORY_PROFILE": (CATEGORY_PROFILE, "standard"),
        "HISTOGRAM_PROFILE": (HISTOGRAM_PROFILE, "analysis"),
        "SAMPLE_PROFILE": (SAMPLE_PROFILE, "presentation"),
    }
    mismatches = [
    ...
    if mismatches:
        raise ValueError(
            "ANALYSIS_PASS=RUN_STABILITY has a fixed bounded profile: "
            + ", ".join(mismatches)
        )
    if ENABLE_SYSTEMATICS:
        raise ValueError("ANALYSIS_PASS=RUN_STABILITY is nominal-only")


def _selector_tag(value):
    normalized = "".join(
    ...
    return f"custom-{hashlib.sha256(value.encode()).hexdigest()[:12]}"
...
if ANALYSIS_PASS == "RUN_STABILITY":
    tag += (
        f"_{RUN_STABILITY_REGION}_OBS-{_selector_tag(RUN_STABILITY_OBSERVABLE_SELECTOR)}"
        f"_CAT-{_selector_tag(RUN_STABILITY_CATEGORY_SELECTOR)}"
    )
