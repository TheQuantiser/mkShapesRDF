"""Validation helpers for compiled ZZ_CR analysis contracts."""
...
def validate_analysis_contract(
    contract, *, cuts, preselections, variables, category_metadata,
    category_variables, expected_context=None
):
    errors = []
    ...
    for category_id, metadata in category_metadata.items():
        ...
        for key in (
            "full_cut_expression", "category_weight_factor", "weight_domain"
        ):
    ...
    for name, definition in variables.items():
        ...
        if canonical_json(compiled.get("range")) != canonical_json(definition.get("range")):
    ...
    for key, expected in (expected_context or {}).items():
        if contract.get(key) != expected:
            errors.append(f"context {key} diverges: {contract.get(key)!r} != {expected!r}")
    if errors:
