...
LUMI_RESULTS = WORKSPACE_ROOT / "lumi" / "results"
EXPECTED_RUNS = {
    "2022": 151,
    "2022EE": 190,
    "2023": 126,
...
def _load(monkeypatch, era="2024", results_dir=LUMI_RESULTS):
    ...
    monkeypatch.setenv("RUN_STABILITY_OBSERVABLES", "all_dy")
    monkeypatch.setenv("RUN_STABILITY_CATEGORIES", "all_dy")
...
def _copy_inputs(tmp_path):
    ...
    shutil.copy2(LUMI_RESULTS.parent / "inputs" / "manifest.json", manifest_path)
    manifest = json.loads(manifest_path.read_text())
    manifest["year_config"] = {
        "path": str((CONFIG_DIR / "year_config.json").relative_to(WORKSPACE_ROOT)),
        "sha256": _sha256(CONFIG_DIR / "year_config.json"),
    }
...
def test_all_eras_have_exact_audited_dy_run_and_output_inventories(
    ...
    for era, expected in EXPECTED_RUNS.items():
        ...
        assert len(contract["observables"]) == 25
        assert len(contract["auxiliary_output_paths"]) == 1200
        assert all(
        ...
        assert contract["mc_source_lumi_fb"] > 0.0
...
def test_live_stale_lumi_manifest_year_config_fails_closed(monkeypatch):
    with pytest.raises(RuntimeError, match="manifest year_config path mismatch"):
        _load(monkeypatch, era="2022", results_dir=LUMI_RESULTS)
...
def test_lumi_manifest_year_config_path_must_name_live_leaf(monkeypatch, tmp_path):
    results = _copy_inputs(tmp_path)
    manifest_path = results.parent / "inputs" / "manifest.json"
    manifest = json.loads(manifest_path.read_text())
    manifest["year_config"]["path"] = (
        "mkShapesRDF/PlotsConfigurationsRun3/ZH_4lMET/ZZ_CR/year_config.json"
    )
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
    provenance_path = results.parent / "provenance.json"
    provenance = json.loads(provenance_path.read_text())
...
def test_lumi_manifest_year_config_identity_must_match_live_leaf(
    monkeypatch, tmp_path
):
    results = _copy_inputs(tmp_path)
    manifest_path = results.parent / "inputs" / "manifest.json"
    manifest = json.loads(manifest_path.read_text())
    manifest["year_config"]["sha256"] = "0" * 64
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
    provenance_path = results.parent / "provenance.json"
    provenance = json.loads(provenance_path.read_text())
    provenance["inputs"]["manifest_sha256"] = _sha256(manifest_path)
    ...
    with pytest.raises(RuntimeError, match="manifest year_config hash mismatch"):
        _load(monkeypatch, results_dir=results)
...
def test_nominal_hlt_path_run_set_mismatch_fails_closed(monkeypatch, tmp_path):
    ...
    def remove_one(rows):
        ...
        for row in rows:
            if (
                not removed
                and row["analysis_era"] == "2024"
                and row["scope_name"] == "HLT_IsoMu24"
            ):
                removed = True
                continue
            kept.append(row)
    ...
    _rewrite_csv(path, remove_one)
    _refresh_receipt(results, path.name)
    with pytest.raises(RuntimeError, match="nominal and hlt_isomu24 run sets differ"):
        _load(monkeypatch, results_dir=results)
