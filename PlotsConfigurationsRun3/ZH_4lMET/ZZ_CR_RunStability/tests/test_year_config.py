...
def test_data_run_filter_is_exact_ordered_and_fails_closed():
    helpers = _helpers()
    full = helpers["load_full_config"]()
    run_tags = helpers["resolve_data_run_tags"](full["years"]["2024"])
    selected = helpers["resolve_data_run_filter"](
        run_tags, {"Run2024I-Prompt-v2", "Run2024C-ReReco-v1"}
    )
    assert selected == ["Run2024C-ReReco-v1", "Run2024I-Prompt-v2"]
    assert helpers["resolve_data_run_filter"](run_tags) == run_tags
    with pytest.raises(ValueError, match="unknown run tags"):
        helpers["resolve_data_run_filter"](run_tags, {"Run2024Z-Unknown-v1"})


def test_2022_singlemuon_is_c_only_with_the_established_trigger_precedence():
    helpers = _helpers()
    year = helpers["load_full_config"]()["years"]["2022"]
    single_muon = next(
        sample for sample in year["data"]["samples"]
        if sample["dataset"] == "SingleMuon"
    )
    assert single_muon == {
        "dataset": "SingleMuon",
        "stream": "Muon",
        "runs": ["Run2022C-ReReco-v1"],
        "trigger": "!Trigger_ElMu && Trigger_sngMu",
    }


def test_data_sample_runs_intersect_the_ordered_global_run_filter():
    helpers = _helpers()
    resolve = helpers["resolve_data_sample_run_tags"]
    sample = {"runs": ["Run2022C-ReReco-v1"]}
    assert resolve(["Run2022C-ReReco-v1", "Run2022D-ReReco-v1"], sample) == [
        "Run2022C-ReReco-v1"
    ]
    assert resolve(["Run2022D-ReReco-v1"], sample) == []
    assert resolve(
        ["Run2022D-ReReco-v1", "Run2022C-ReReco-v1"], {}
    ) == ["Run2022D-ReReco-v1", "Run2022C-ReReco-v1"]


def test_per_sample_run_and_trigger_overrides_fail_closed():
    helpers = _helpers()
    raw = json.loads(YEAR_CONFIG.read_text())
    sample = raw["years"]["2022"]["data"]["samples"][0]
    sample["runs"] = ["Run2022Z-Unknown-v1"]
    with pytest.raises(ValueError, match="runs contains unknown run tags"):
        helpers["_materialize_years"](raw)

    raw = json.loads(YEAR_CONFIG.read_text())
    sample = raw["years"]["2022"]["data"]["samples"][0]
    sample["trigger"] = "Trigger_NotConfigured"
