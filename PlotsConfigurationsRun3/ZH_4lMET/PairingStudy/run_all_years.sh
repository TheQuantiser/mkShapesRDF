...
run_compile_or_analysis() {
  local year="$1"
  local configs_dir="${SCRIPT_DIR}/configs/${PAIRING_CAMPAIGN}/${year}"
  local output_dir="${SCRIPT_DIR}/rootFiles/${PAIRING_CAMPAIGN}/${year}"
  export YEAR="${year}"
  export PAIRING_CONFIGS_FOLDER="${configs_dir}"
  export PAIRING_OUTPUT_FOLDER="${output_dir}"
  export PAIRING_BATCH_FOLDER="${SCRIPT_DIR}/condor/${PAIRING_CAMPAIGN}/${year}"

  echo "[PairingStudy] mode=${MODE} campaign=${PAIRING_CAMPAIGN} YEAR=${YEAR}"
  case "${MODE}" in
    compile)
...
case "${MODE}" in
  ...
  *)
    for year in "${YEARS[@]}"; do
      run_compile_or_analysis "${year}"
    done
    ;;
esac

echo "[PairingStudy] completed mode=${MODE} campaign=${PAIRING_CAMPAIGN}"
