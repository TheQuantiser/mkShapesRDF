...
from pairing_config import SUPPORTED_YEARS, load_pairing_year  # noqa: E402
...
class RootReader:
    ...
    def __init__(self, inputs, warnings, strict=False):
        ...
        for year, path in inputs.items():
            ...
            if not handle or handle.IsZombie():
                raise OSError(f"Cannot open YEAR={year} ROOT input: {path}")
    ...
    def read(self, year, baseline, variable, sample, shape, required=False):
        ...
        if not histogram:
            message = f"YEAR={year}: missing {path}"
        ...
        if histogram.GetDimension() == 1:
            if histogram.GetNbinsX() != expected:
                message = (
                    f"YEAR={year}: {path} has {histogram.GetNbinsX()} unrolled bins; "
            ...
            if actual != tuple(shape):
                raise ValueError(f"YEAR={year}: {path} shape {actual}, expected {shape}")
            result = [0.0] * expected
            if len(shape) == 2:
            ...
            raise ValueError(
                f"YEAR={year}: {path} is dimension {histogram.GetDimension()}, "
...
def _discover_inputs(root):
    ...
    for year in SUPPORTED_YEARS:
        ...
        if len(candidates) > 1:
            ...
            raise RuntimeError(
                f"Ambiguous merged ROOT files for YEAR={year} below {root}:\n  {details}\n"
                "Pass explicit YEAR=ROOT inputs."
            )
...
def _parse_input_specs(specs):
    ...
    for spec in specs:
        if "=" not in spec:
            raise ValueError(f"Input must be YEAR=ROOT, got {spec!r}")
        ...
        if not path:
            raise ValueError(f"Empty ROOT path for YEAR={year}")
        if year in result:
            raise ValueError(f"YEAR={year} was supplied more than once")
        if not path.startswith("root://") and not Path(path).expanduser().is_file():
            raise FileNotFoundError(f"YEAR={year} input does not exist: {path}")
        ...
        for sample_label, samples in _sample_groups(inventories[year], family):
            for baseline in baselines:
                ...
                if cubes["raw"] is None:
                    warnings.append(
                        f"YEAR={year} family={family} sample={sample_label} baseline={baseline}: "
...
def _parser():
    ...
    parser.add_argument(
        "inputs", nargs="*", metavar="YEAR=ROOT",
        help="explicit merged input; may be repeated (also accepted via --input)",
    )
    parser.add_argument("-i", "--input", action="append", default=[], metavar="YEAR=ROOT")
    parser.add_argument(
        "--input-root", default=os.environ.get("PAIRING_OUTPUT_ROOT"),
        help="campaign directory to auto-discover when no YEAR=ROOT is supplied",
    )
...
def main(argv=None):
    ...
    inputs = {year: inputs[year] for year in SUPPORTED_YEARS if year in inputs}
    ...
    inventory_payload = {
        ...
        "complete_run3": tuple(inputs) == tuple(SUPPORTED_YEARS),
        "missing_years": [year for year in SUPPORTED_YEARS if year not in inputs],
