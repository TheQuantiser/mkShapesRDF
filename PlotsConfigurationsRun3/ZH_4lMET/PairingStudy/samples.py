...
PAIRING_YEAR = load_pairing_year(globals().get("YEAR") or os.environ.get("YEAR"))
YEAR = PAIRING_YEAR["year"]
...
PAIRING_SAMPLE_INVENTORY = {
    "year": YEAR,
    "production": PAIRING_YEAR["production"],
    "steps": PAIRING_YEAR["steps"],
    "lumi_fb": PAIRING_YEAR["lumi_fb"],
...
def _discover(component):
    ...
    if not files:
        raise RuntimeError(
            f"YEAR={YEAR} found no files for source={source!r} under {directory!r}"
        )
    ...
    if not files:
        raise RuntimeError(
            f"YEAR={YEAR} source={source!r} became empty after file limiting"
        )
    return list(files), directory, available_file_count


for _family in ("ZH", "ZZ"):
    for _logical_sample in PAIRING_YEAR["inventory"][_family]:
        _sample_components = []
        _inventory_components = []
        for _component in PAIRING_YEAR["logical_components"][_logical_sample]:
...
if set(samples) != set(
    PAIRING_YEAR["inventory"]["ZH"] + PAIRING_YEAR["inventory"]["ZZ"]
):
...
print(f"[PairingStudy] YEAR={YEAR}")
print(
    f"[PairingStudy] production={PAIRING_YEAR['production']} "
    f"steps={PAIRING_YEAR['steps']}"
)
for _family in ("ZH", "ZZ"):
    _aliases = tuple(PAIRING_YEAR["inventory"][_family])
