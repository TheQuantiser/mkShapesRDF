...
ROOT::VecOps::RVec<int> xPairIdxWithID(
  ...
  if (zidx[0] >= n || zidx[1] >= n) return {-1, -1};

  auto isZ = [&](int i){ return i == zidx[0] || i == zidx[1]; };
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = i + 1; j < n; ++j) {
      ...
      if (!pairPassesSelection(cand,
                               pt, pdgId,
                               passEleWP, passMuWP,
                               minPassID, leadPtMin, subleadPtMin))
        continue;

      const float leadPt = pt[cand[0]];
      const float subPt  = pt[cand[1]];

      if (leadPt > bestLeadPt || (!(leadPt < bestLeadPt) && subPt > bestSubPt)) {
        bestLeadPt = leadPt;
        bestSubPt  = subPt;
        best = cand;
      }
