...
def _load_state(year, is_data, available_branches):
    ...
    for filename in ("year_config.py", "selection_config.py", "aliases.py", "cuts.py"):
...
def count_input(args):
    ...
    try:
        ...
        parent_actions = {}
        for region in dict.fromkeys(item["physics_region"] for item in metadata.values()):
            parent = next(
        ...
        parent_counts = {name: int(action.GetValue()) for name, action in parent_actions.items()}
        category_counts = {
