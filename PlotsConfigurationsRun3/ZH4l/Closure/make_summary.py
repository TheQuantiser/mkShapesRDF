...
NMINUS_VARIABLE = {
    ...
    "N1_NO_BVETO": "tagged_jet_count",
    "N1_NO_LOWMASS": "zx_min_pair_mass",
    "N1_NO_FIFTHVETO": "extra_tight_lepton_count",
    "N1_NO_4LPT": "zx_lepton_rank1_pt",
    "N1_NO_ZWINDOW": "z_mass",
}
...
class Reader:
    ...
    def samples(self, era, stage="S8_Z_BRIDGE", variable="yield"):
        ...
        return tuple(
            ...
            if key.GetName().startswith("histo_")
            and not key.GetName().endswith(("Up", "Down"))
        )

    def categories(self, era):
        return tuple(
            key.GetName()
            for key in self.files[era].GetListOfKeys()
            if key.GetName() != "trees" and key.GetClassName().startswith("TDirectory")
        )

    def variables(self, era, category):
        directory = self.files[era].Get(category)
        return (
            tuple(key.GetName() for key in directory.GetListOfKeys())
            if directory
            else ()
        )
        ...
        for era in eras:
            for sample in samples:
                ...
                if bins is None:
                    ...
                    edges = [h.GetXaxis().GetBinLowEdge(i) for i in range(1, n + 1)] + [
...
def process_groups():
    ...
    out = {
        "DY": set(groups["DY"]["samples"]),
        "ZZ": set(groups["ZZ"]["samples"]),
        "WZ": set(groups["WZ"]["samples"]),
        "Vg/VgS": set(groups["Vg"]["samples"]) | set(groups["VgS"]["samples"]),
        ...
        "Higgs": set().union(
            *(
                set(groups[name]["samples"])
                for name in ("HWW_signal", "HWW_contamination", "H_other")
            )
        ),
    }
    return out
...
def chi2_metrics(data, mc):
    if not data or not mc or len(data["bins"]) != len(mc["bins"]):
        return None
    valid = [
        i for i in range(len(data["bins"])) if data["binvars"][i] + mc["binvars"][i] > 0
    ]
    if not valid:
        return None
    pulls = [
        (data["bins"][i] - mc["bins"][i])
        / math.sqrt(data["binvars"][i] + mc["binvars"][i])
        for i in valid
    ]
    ...
    return {
        "absolute_chi2_ndf": absolute / max(1, len(valid)),
        "shape_chi2_ndf": shape,
        "max_abs_bin_pull": max(map(abs, pulls)),
        "bins_used": len(valid),
    }
...
def build(inputs, output):
    ...
    try:
        ...
        combinations.update(
            {
                name: tuple(era for era in eras if era in inputs)
                for name, eras in COMBINED_ERAS.items()
            }
        )
        combinations = {name: eras for name, eras in combinations.items() if eras}
        all_samples = {era: reader.samples(era) for era in inputs}
        mc_by_era = {
            era: tuple(sample for sample in samples if sample != "DATA")
            for era, samples in all_samples.items()
        }
        ...
        for label, eras in combinations.items():
            mc_samples = tuple(
                dict.fromkeys(sample for era in eras for sample in mc_by_era[era])
            )
            for stage in STAGE_ORDER:
                data = reader.aggregate(eras, stage, "yield", ("DATA",))
                mc = reader.aggregate(eras, stage, "yield", mc_samples)
                data_base = reader.aggregate(eras, stage, "yield_base", ("DATA",))
                mc_base = reader.aggregate(eras, stage, "yield_base", mc_samples)
                ...
                stage_rows.append(
                    {
                        ...
                        "N_DATA": data["value"],
                        "N_MC": mc["value"],
                        "MC_stat": math.sqrt(max(0.0, mc["variance"])),
                        "DATA_MC_full": ratio(data["value"], mc["value"]),
                        ...
                        "stat_pull": (
                            None
                            if denom == 0
                            else (data["value"] - mc["value"]) / denom
                        ),
                ...
                for group, configured in groups.items():
                    result = reader.aggregate(
                        ...
                        stage,
                        "yield",
                        tuple(sample for sample in mc_samples if sample in configured),
                    )
                    ...
                    composition_rows.append(
                        {
                            ...
                            "non_DY_prompt_gt_10pct": bool(
                                group != "DY" and total and value / total > 0.1
                            ),
                            "fake_limitation_flag": stage
                            in (
                                "S4_NO_BVETO",
                                "S5_NO_LOWMASS",
                                "S6_NO_FIFTHVETO",
                        ...
                        distributions.append(
                            {
                                ...
                                "edges": dshape["edges"],
                                "data": dshape["bins"],
                                "data_variance": dshape["binvars"],
                                "mc": mcshape["bins"],
                                "mc_variance": mcshape["binvars"],
                            }
                        )
            categories = tuple(
                dict.fromkeys(
                    category for era in eras for category in reader.categories(era)
                )
            )
            for category in categories:
                variables = tuple(
                    dict.fromkeys(
                        variable
                        for era in eras
                        for variable in reader.variables(era, category)
                    )
                )
                    ...
                    category_rows.append(
                        {
                            ...
                            "N_DATA": data["value"],
                            "N_MC": mc["value"],
                            "DATA_MC": ratio(data["value"], mc["value"]),
                        }
                    )
                        ...
                        distributions.append(
                            {
                                ...
                                "edges": data["edges"],
                                "data": data["bins"],
                                "data_variance": data["binvars"],
                                "mc": mc["bins"],
                                "mc_variance": mc["binvars"],
                            }
                        )
            for tight, loose in TRANSITIONS:
                dt = reader.aggregate(eras, tight, "yield_base", ("DATA",))
                dl = reader.aggregate(eras, loose, "yield_base", ("DATA",))
                mt = reader.aggregate(eras, tight, "yield_base", mc_samples)
                ml = reader.aggregate(eras, loose, "yield_base", mc_samples)
                if not all((dt, dl, mt, ml)):
                    continue
                ed, em = ratio(dt["value"], dl["value"]), ratio(
                    mt["value"], ml["value"]
                )
                ...
                if (
                    ...
                    and ed
                    and em
                    and ed_variance is not None
                    and em_variance is not None
                ):
                    kappa_variance = (
                        kappa
                        * kappa
                        * (ed_variance / (ed * ed) + em_variance / (em * em))
                    )
                transition_rows.append(
                    {
                        ...
                        "epsilon_DATA": ed,
                        "epsilon_DATA_stat": (
                            None if ed_variance is None else math.sqrt(ed_variance)
                        ),
                        "epsilon_MC_BASE": em,
                        "epsilon_MC_BASE_stat": (
                            None if em_variance is None else math.sqrt(em_variance)
                        ),
                        "kappa": kappa,
                        "kappa_stat": (
                            None
                            if kappa_variance is None
                            else math.sqrt(max(0.0, kappa_variance))
                        ),
            ...
            for stage in WEIGHT_SENTINELS:
                ...
                for step, variable in (
                    ("base", "yield_base"),
                    ("selected_lepton", "yield_lepton"),
                    ("selected_lepton_trigger", "yield_lepton_trigger"),
                    ("full", "yield_nominal"),
                ):
            ...
            for scope, both_category, current_only_category, event_only_category in (
                ...
                for population, category in (
                    ...
                    dpop = reader.aggregate(eras, category, "z_mass", ("DATA",))
                    mpop = reader.aggregate(eras, category, "z_mass", mc_samples)
                    if dpop and mpop:
                ...
                if set(populations) == {"both", "current_only", "eventPt_only"}:
                    for sample_kind, index in (("DATA", 0), ("MC", 1)):
                        values = {
                            name: pair[index]["value"]
                            for name, pair in populations.items()
                        }
                        ...
                        pt_contract_rows.append(
                            {
                                "era": label,
                                "scope": scope,
                                "population": "migration_fraction",
                                "sample_kind": sample_kind,
                                "N_union": union,
                                "N_migrated": migrated,
                                "fraction_migrated": ratio(migrated, union),
                            }
                        )
        ...
        (output / "distributions.json").write_text(
            json.dumps(distributions, indent=2, sort_keys=True, allow_nan=False) + "\n"
        )
        metadata = {
            "inputs": inputs,
            "eras_and_combinations": combinations,
            "nonprompt_fake_background_included": False,
            "statement": "Nonprompt/fake background is not included.",
            "statistical_scope": "nominal/stat-only; no systematic-significance p-values",
        }
