...
from common.naming import historical_expression
...
def _digest(expression):
    return hashlib.sha256(historical_expression(expression).encode()).hexdigest()
...
def test_exact_audited_reference_cut_expressions_are_frozen_locally():
    ...
    assert (
        _digest(PRESELECTION)
        == "497e7155419122ff181775e9aed833d5bc8f98876294f03c8570ba79ef767f02"
    )
    assert (
        _digest(EXACT_ZZCR)
        == "f8a89f4b8376c5690280e2776b633e7078b8b0d4e6867d63aa7b7d9192d5f214"
    )
    assert (
        _digest(CURRENT_DY)
        == "cd1f19ad2fee6d5158eb8ee1684cabdec261303c093304d8f439e3152c4d83e7"
    )
    assert (
        _digest(CURRENT_DY_ENRICHED)
        == "f8ff8df757849549ba0f16ba3945bd727b7bb627b2c6cae490d9ad813998cbd2"
    )
