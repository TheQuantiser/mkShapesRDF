...
def test_dy_axes_are_finer_than_four_lepton_axes():
    ...
    for observable in ("mZ", "ptZ", "phiEtaStar", "PuppiMET_pt"):
...
def test_compact_physics_binning_contract():
    ...
    assert definition("N1_NO_ZWINDOW", "mZ")["range"] == (24, 30.0, 150.0)
    assert definition("S7_FOURL_BRIDGE", "mX")["range"] == (20, 0.0, 200.0)
    assert definition("N1_NO_XMASS", "mX")["range"] == (20, 0.0, 200.0)
    ...
    assert definition("S0_ZZCR", "ptX")["range"] == definition("S0_ZZCR", "ptZ")["range"]
