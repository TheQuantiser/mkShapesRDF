...
run_era() {
  ...
  local pickle
  set_era_paths "${era}"
  echo "[Closure] mode=${MODE} campaign=${CLOSURE_CAMPAIGN} ERA=${era}"
  case "${MODE}" in
    inspect)
...
case "${MODE}" in
  summary)
    inputs=()
    for era in "${ERAS[@]}"; do
      root_file="$(find "${SCRIPT_DIR}/rootFiles/${CLOSURE_CAMPAIGN}/${era}" -maxdepth 1 -type f -name 'mkShapes__*.root' | sort | tail -n 1)"
      if [[ -z "${root_file}" ]]; then
        echo "No merged ROOT file found for ${era}" >&2
        exit 1
      fi
...
echo "[Closure] completed mode=${MODE} campaign=${CLOSURE_CAMPAIGN}"
