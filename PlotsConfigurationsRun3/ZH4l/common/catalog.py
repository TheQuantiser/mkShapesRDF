"""Materialize the validated ZH4l process catalogue into ``samples``.

Leaves execute this file after declaring ``CORRECTION_WEIGHT``.  Input
discovery remains the native :class:`SearchFiles` implementation.
"""
...
YEAR = ERA  # compatibility value exported only for downstream framework code
_resolved_overlap = resolve_overlap_model(_selected_era, _full_config)
mcProduction = _selected_era["mc"]["production"]
mcSteps = _selected_era["mc"]["steps"]
dataReco = _selected_era["data"]["reco"]
dataSteps = _selected_era["data"]["steps"]


CORRECTION_WEIGHT = str(globals().get("CORRECTION_WEIGHT", "puWeight"))
if not CORRECTION_WEIGHT.strip():
    raise RuntimeError("CORRECTION_WEIGHT must be a non-empty RDF expression")
...
try:
    _resolved_sample_profile = resolve_sample_selection(
        ...
        _full_config,
        SAMPLE_PROFILE,
        globals().get("SAMPLE_FILTER")
        or (os.environ.get("SAMPLE_FILTER") if "SAMPLE_FILTER" in os.environ else None),
    )
except ValueError as _sample_scope_error:
    raise RuntimeError(
        f"Invalid sample scope for ERA={ERA}: {_sample_scope_error}"
...
def makeMCDirectory(sample_name, var=""):
    _treeBaseDir = _with_redirector(
        resolve_tree_base_dir(_selected_era, "mc", sample_name=sample_name)
    )
    if var == "":
...
mcCommonWeight = _selected_era["mc"].get("common_weight", "XSWeight")
for mc_sample in _resolved_overlap["passthrough_sources"]:
    ...
    _source_norm = source_normalization(mc_sample, _selected_era, _full_config)
    if _source_norm != 1.0:
        files = [(files[0][0], files[0][1], f"({_source_norm:.16g})")]
    samples[mc_sample] = {
        "name": files,
        "weight": mcCommonWeight + "*" + CORRECTION_WEIGHT,
...
DataRunTags = resolve_data_run_tags(_selected_era)
_data_stream_filter = {
    item.strip()
...
try:
    DataSamples = resolve_data_samples(_selected_era, _data_stream_filter)
except ValueError as _data_filter_error:
    raise RuntimeError(
        f"Invalid DATA_STREAM_FILTER for ERA={ERA}: {_data_filter_error}"
    ) from _data_filter_error

if _sample_enabled("DATA"):
    samples["DATA"] = {
        "name": [],
        "weight": _selected_era["data"].get("common_weight", "METFilter_DATA"),
        "weights": [],
        "isData": ["all"],
        "FilesPerJob": filesPerJob,
    }
