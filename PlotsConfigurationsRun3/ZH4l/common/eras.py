"""Validated era metadata and materialization for the ZH4l family.

The JSON schema retains its historical ``years`` keys so that the validated
payload is byte-for-byte comparable with the legacy source.  They represent
Run-3 eras/periods.  The public selector and all new code use ``ERA``.
"""
...
def resolve_era(era=None, *, environ=None):
    """Resolve the preferred ``ERA`` selector and checked ``YEAR`` fallback.

    An explicitly supplied *era* has highest priority.  ``YEAR`` remains a
    transition alias, but inconsistent values fail closed.
    """
    env = os.environ if environ is None else environ
    requested = None if era is None else str(era)
    era_env = env.get("ERA")
    year_env = env.get("YEAR")
    if era_env and year_env and era_env != year_env:
        raise ValueError(
            f"Inconsistent ERA={era_env!r} and compatibility YEAR={year_env!r}"
        )
...
def load_selected_era(config_filename="eras.json", era=None):
    """Return ``(era, era_cfg, full_cfg)`` for the active Run-3 era."""
    full_cfg = load_full_config(config_filename)

    year_key = resolve_era(era) or full_cfg["default_year"]
    available_years = sorted(full_cfg["years"])
    if year_key not in full_cfg["years"]:
        raise ValueError(
            f"Unsupported ERA={year_key!r}. Available eras: {available_years}"
        )
...
def load_full_config(config_filename="eras.json"):
    """Return the materialized configuration with shared defaults expanded."""
    return _load_year_config(config_filename)


def load_selected_year(config_filename="eras.json", env_var="YEAR"):
    """Compatibility alias for legacy callers; new code must use ERA."""
    if env_var != "YEAR":
        raise ValueError("Only the compatibility YEAR selector is supported")
    return load_selected_era(config_filename)
